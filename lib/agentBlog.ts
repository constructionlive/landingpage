...
/* Server-only helpers shared by the /api/blog route handlers. They stand between
   the editorial agent's bearer-authenticated HTTP calls and the secret-authed
   Convex functions in convex/posts.ts. The bearer the caller sends is forwarded
   as `apiKey`; Convex is what actually verifies it, so a leaked or missing
   secret fails at the source, not just here. */
