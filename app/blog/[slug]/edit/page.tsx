...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    setTwitterCard(post.twitterCard ?? "");
    setTwitterTitle(post.twitterTitle ?? "");
    setTwitterDescription(post.twitterDescription ?? "");
    setTwitterImageUrl(post.twitterImageUrl ?? "");
    if (post.metaTitle || post.metaDescription || post.metaKeywords || post.canonicalUrl || post.noIndex || post.ogTitle || post.ogDescription || post.ogImageUrl || post.twitterCard || post.twitterTitle || post.twitterDescription || post.twitterImageUrl) {
      setSeoOpen(true);
    }
    setLoaded(true);
  }, [post, loaded]);
  ...
  const isOwner = post && me ? post.authorId === me.id : false;
  const canEdit = isWriter && (isOwner || role === "admin");

  if (post === undefined) {
    return (
      <main className="mx-auto min-h-screen w-full max-w-3xl px-6 py-12 bg-do-bg">
        <p className="text-sm text-do-text-secondary">Loading post...</p>
      </main>
    );
  }
  ...
  if (post === null) {
    return (
      <main className="mx-auto min-h-screen w-full max-w-3xl px-6 py-12 bg-do-bg">
        <h1 className="text-4xl font-bold text-do-text">Post Not Found</h1>
        <Link href="/blog" className="mt-4 inline-block text-sm text-do-orange hover:underline">
          Back to blog
        </Link>
      </main>
    );
  ...
  if (!canEdit) {
    return (
      <main className="mx-auto min-h-screen w-full max-w-3xl px-6 py-12 bg-do-bg">
        <h1 className="text-4xl font-bold text-do-text">Access Denied</h1>
        <p className="mt-2 text-sm text-do-text-secondary">
          You must be the author or an admin to edit this post.
        </p>
        <Link href={`/blog/${slug}`} className="mt-4 inline-block text-sm text-do-orange hover:underline">
          Back to post
        </Link>
      </main>
    );
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await updatePost({
        ...
        slug: postSlug.trim() ? postSlug.trim() : undefined,
      ...
      const targetSlug = postSlug.trim() || slug;
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-3xl px-6 py-12 bg-do-bg">
      ...
      <h1 className="text-4xl font-bold text-do-text">Edit Post</h1>
      <p className="mt-2 text-sm text-do-text-secondary">
        Editing &ldquo;{post.title}&rdquo;
      </p>
      <form className="mt-8 flex flex-col gap-4" onSubmit={onSubmit}>
        <input className="rounded-md border border-do-border bg-transparent px-3 py-2 text-sm text-do-text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} required />
        <input className="rounded-md border border-do-border bg-transparent px-3 py-2 text-sm text-do-text" placeholder="Slug (optional)" value={postSlug} onChange={(e) => setPostSlug(e.target.value)} />
        <textarea className="min-h-24 rounded-md border border-do-border bg-transparent px-3 py-2 text-sm text-do-text" placeholder="Excerpt (optional)" value={excerpt} onChange={(e) => setExcerpt(e.target.value)} />
        <div className="flex flex-col gap-2 rounded-md border border-do-border p-3">
          ...
          <input className="rounded-md border border-do-border bg-transparent px-3 py-2 text-sm text-do-text" placeholder="https://..." value={coverImageUrl} onChange={(e) => setCoverImageUrl(e.target.value)} />
          <div className="flex items-center gap-3">
            <label className="cursor-pointer rounded-md border border-do-border px-3 py-2 text-xs text-do-text hover:bg-do-bg-light/50">
              ...
              <input className="hidden" type="file" accept="image/*" disabled={uploadingCoverImage} onChange={(e) => { const file = e.target.files?.[0]; if (!file) return; onCoverImageSelect(file); e.target.value = ""; }} />
            </label>
            {coverImageUrl ? <span className="truncate text-xs text-do-text-secondary">Selected: {coverImageUrl}</span> : null}
          </div>
        </div>
        <BlogEditor value={content} onChange={setContent} onUploadImage={uploadImage} onError={setError} />
        <section className="rounded-md border border-do-border p-4">
          <button type="button" className="flex w-full items-center justify-between text-left" onClick={() => setSeoOpen((o) => !o)}>
            <div>
              <h2 className="text-sm font-semibold text-do-text">SEO &amp; Meta Tags (optional)</h2>
              <p className="mt-1 text-xs text-do-text-secondary">Customize meta title, description, Open Graph, Twitter cards, and more.</p>
            </div>
        ...
        <section className="rounded-md border border-do-border p-4">
          ...
          <p className="mt-1 text-xs text-do-text-secondary">Set your name, profile image, and short bio for blog attribution.</p>
